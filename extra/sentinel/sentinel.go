...
func NewClientWithLogger(
	...
	*Client, error,
	...
	initLogger.Infof("Setting up with network:%s, addr:%s, masterNames:%v, sentinelTimeouts:%v, "+
		"redisTimeouts:%v, heartbeatPeriod:%s, softMaxPoolSize:%d",
		network, address, names, sentinelTimeouts, redisTimeouts, heartbeatPeriod, poolSize)
	//
	// Connect to sentinel
	...
	// while reading. Instead of read deadlines, we use heart beats.
	subClient.Client.SetReadTimeout(time.Duration(0))
	...
	for {
		...
		case req := <-c.getCh:
			...
			if alwaysErr != nil {
				...
				req.retCh <- &getReqRet{nil, alwaysErr}
				continue
			}
