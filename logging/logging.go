...
// Note: it's not safe for the prefix string to contain the '%' character, as it will interfere
//       with the fmt parsing of messages.
...
func (l *LoggerWithPrefix) WrappedLogger() SimpleLogger {
	return l.wrappedLogger
}

// WithAnotherPrefix returns a new LoggerWithPrefix that keeps the same wrapped logger,
// but concatenates another prefix
